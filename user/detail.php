...
//ambil data content
$get = $config->query("SELECT c.id_lokasi AS id_lokasi, nm_lokasi, informasi, AVG(rating) AS rating, gambar FROM df_wisata c LEFT JOIN t_rating r ON(c.id_lokasi = r.id_lokasi) WHERE c.id_lokasi = '" . $_GET['id'] . "'");
...
$cek    = $config->query("SELECT * FROM `t_rating` WHERE  id_lokasi = '" . $_GET['id'] . "' AND id_user = '" . $id_user . "'");
...
if ($cek->num_rows > 0) {
    ...
    //jika diinputkan maka lakukan explode data dan memasukkan data ke variabel fasilitas dan tempat
...
<body>
    <div class="container">
        ...
        <div class="row mt-5">
            ...
            <div class="col-md-7">
                <h4><?php echo $d['nm_lokasi']; ?></h4>
                <div class="star">
                    <?php echo $star; ?> (<?php echo ($d['rating'] != '') ? (int)$d['rating'] : 0; ?>/5)
                </div>
                <div class="clearfix"></div>
                <p align="justify" class="lead"><?php echo $d['informasi']; ?></p>
            </div>
            ...
            <div class="col-lg-10">
                ...
                <h4>Komentar</h4>
                <div id="komentar-lista" class="row">

                    <!-- tampilkan komentar -->
                    <?php
                    $komentar = mysqli_query($config, "SELECT rating, komentar, nama_user FROM t_rating  WHERE id_lokasi = '" . $_GET['id'] . "'");
                    ...
                    while ($komen = $komentar->fetch_assoc()) {
                        ...
                        for ($i = 1; $i <= 5; $i++) {
                            if ($i <= ceil($komen['rating'])) {
                                ...
                                '.'<hr>'. $komen['nama_user']. '<br />' . $star_rating . '<br />' . $komen['komentar'] . '
                                </div>';
    ...
    <hr />
            <div class="text-right mt-4">
                <a href="?page=wisata_alam" class="btn btn-secondary">Kembali</a>
                <a href="?page=rute&id=<?= $d['id_lokasi'] ?>" class="btn btn-primary">Rute</a>
            </div>
           </div>
            <div class="col-lg-12">
                <div class="row mt-4">
                    <h5>Silahkan Berikan Penilaian Anda</h5>
                </div>

                <div class="row">
                    <form method="POST" action="./rate/proses.php">
                        <input type="hidden" name="id" id="IDContent" value="<?php echo $d['id_lokasi']; ?>">
                        ...
                        <div class="form-group perbaikan" style="display: none;">
                            ...
                            <div class="checkbox">
                                <label>
                                    <input type="checkbox" name="perbaikan[]" value="Lainya" <?= (isset($exp_perbaikan) && in_array('Lainya', $exp_perbaikan)) ? 'checked' : ''; ?>> Lainnya
                                </label>
                            </div>
                        </div>
