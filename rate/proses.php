...
if (isset($_POST['rating']) && is_numeric($_POST['rating']) && isset($_POST['id']) && is_numeric($_POST['id'])) {
    ...
    $sql = $config->query("SELECT * FROM `t_rating` WHERE id_lokasi = '" . $id . "' AND id_user = '" . $id_user . "'");

    //hitung row
    if ($sql->num_rows > 0) {

        //lakukan update jika user sudah pernah menilai sesuai id content
        $config->query("UPDATE `t_rating`   SET  `rating` = '" . $rating . "', `perbaikan` = '" . $fas . "', `nama_user` = '" . $nama_user . "', `komentar` = '" . $komentar . "'  WHERE id_user = '" . $id_user . "' AND id_lokasi = '" . $id . "'");
    } else {
        //simpan jika user belum pernah menilai
        $config->query("INSERT INTO `t_rating` VALUES ('" . $id_user . "', '" . $rating . "', '" . $fas . "', '" . $id . "', '" . $nama_user . "','" . $komentar . "')");
    }
